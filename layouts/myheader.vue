            ...
            <div class="bottom wechat" style="margin-top: -80px;">
              <div class="phone-container">
                <div class="phone-wrapper" @click="phoneLogin()"><span
                  class="iconfont icon"></span></div>
                <span class="third-text"> 手机短信或者邮箱验证码登录 </span></div>
            </div>
          </div>
        </div>
        <!-- 微信登录 #end -->
...
const defaultDialogAtrr = {
  ...
  inputValue: '', // 输入框绑定对象
  placeholder: '请输入您的手机号或者邮箱', // 输入框placeholder
  maxlength: 25, // 输入框长度控制
...
export default {
  ...
  methods: {
    ...
    getCodeFun() {
      ...
      if (emailRegex.test(this.userInfo.phone)) {
        // 邮箱格式不正确，进行相应处理
        console.log('邮箱格式正确');
      } else {
        if (!(/^1[34578]\d{9}$/.test(this.userInfo.phone))) {
          this.$message.error('手机号码或者邮箱格式不正确')
          return;
        }
      }
